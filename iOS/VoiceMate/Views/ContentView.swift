...
struct ContentView: View {
    ...
    private var inputBar: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                // Always render both views together so @FocusState binding is never orphaned.
                // This fixes the bug where keyboard won't reappear after dismissal via '完成' or tap-blank.
                ZStack {
                    textFieldArea
                        .opacity(inputMode == .text ? 1 : 0)
                        .allowsHitTesting(inputMode == .text)
                    
                    voiceButtonArea
                        .opacity(inputMode == .voice ? 1 : 0)
                        .allowsHitTesting(inputMode == .voice)
                }
